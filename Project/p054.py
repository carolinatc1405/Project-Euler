# 
# Solução para o problema do Projeto Euler 54
# 
...
# O "handpair" é um conjunto de 10 cartas separadas por espaço.
def is_player1_win(handpair):
	# Parse as cartas e dividir entre os jogadores
	cards = [parse_card(item) for item in handpair.split(" ")]
	assert len(cards) == 10
	player1 = cards[ : 5]
	player2 = cards[5 : ]
	# Compare as pontuações das mãos
	return get_score(player1) > get_score(player2)


# A mão é um array de cartas. Retorna uma pontuação para a mão dada. Se handX bate handY então get_score(handX) > get_score(handY),
# e se handX é um empate com handY então get_score(handX) = get_score(handY) (mesmo que as mãos tenham cartas diferentes).
# Oss resultados não precisam ser consecutivos - por exemplo, mesmo que os resultados 1 e 3 existam, pode não haver
# mão que produz uma pontuação de 2. A propriedade de comparação é a única garantia fornecida por get_score().
def get_score(hand):
	assert len(hand) == 5
	
	# número de cartas com o valor de i
	rankcounts = [sum(1 for (rank, _) in hand if rank == i) for i in range(13)]
	
	# rankcounthist[i] é o número de vezes que ocorre uma contagem de i.
	# Por exemplo, se houver exatamente uma trinca, então rankcounthist[3] = 1.
	rankcounthist = [rankcounts.count(i) for i in range(6)]
	
	# flushsuit está no index [0,3] se todas as cartas tiverem esse index; caso contrário -1
	...
	#Ideia principal: Codificar o tipo de mão nos bits superiores, depois codificar até 5 cartas em big-endian (4 bits cada).
	if   straighthighrank != -1 and flushsuit != -1     : return 8 << 20 | straighthighrank  
	elif rankcounthist[4] == 1                          : return 7 << 20 | bestcards         
	elif rankcounthist[3] == 1 and rankcounthist[2] == 1: return 6 << 20 | bestcards         
	elif flushsuit != -1                                : return 5 << 20 | bestcards
	elif straighthighrank != -1                         : return 4 << 20 | straighthighrank  
	elif rankcounthist[3] == 1                          : return 3 << 20 | bestcards
	elif rankcounthist[2] == 2                          : return 2 << 20 | bestcards         
	elif rankcounthist[2] == 1                          : return 1 << 20 | bestcards        
	else                                                : return 0 << 20 | bestcards         


# Codifica 5 cartões em 20 bits em big-endian, começando com os cartões mais freqüentes,
# rompendo os laços por ordem superior. 
...
# Retorna a pontuação da carta mais alta na reta, ou -1 se o conjunto de cartas não formar uma reta.
# Leva em conta o fato de que o ás pode ser de valor 0 (ou seja, valor de face 1) ou de valor 13 (valor imediatamente após o rei).
...
# A carta é uma sequência de 2 letras como "2H" para "dois de copas". Devolve um par de números inteiros (rank, suit).
def parse_card(card):
